...
> **Last updated:** 2026-08-23 (504 remediation resolved — see [improvement.md](improvement.md); `/predict-ai` given a `withBudget` ceiling + `expire` 900→3600 after a **fourth** external 504 report, whose root cause was wrong but whose measurement was not — Trap 12 and "Report #4" there; Verified tier application submitted; `polybets.xyz` confirmed live) · **Phase:** Milestone 1 substantially complete (auth, geo-gate, fee engine, Deposit Wallet + deposit UI, pre-trade authorization, client-side market-order signing all built and verified on local workerd) · Weeks 2–4 (**Market Discovery, Trading Engine & WebSockets, Portfolio/Testing & Launch**) now being executed as **one combined build phase** — see the Milestones section below · **Repo:** on `feat/copy-trade`
>
...
scripts/
  ...
  warm-cache.mjs           post-deploy cache warmup (built 2026-08-23) — runs automatically at the end of `npm run deploy`; never fails the deploy
...
- **Every deploy orphans the entire R2 cache.** OpenNext's `getR2Key` composes every key with `OPEN_NEXT_BUILD_ID`, so publishing invalidates everything at once and the first visitor to each route rebuilds inline. **A 504 burst in the ~30 min after a deploy is expected and self-limiting** — 49 errors across 15 paths, measured. `npm run deploy` now ends with `scripts/warm-cache.mjs`, which pays that cost for us (23 paths in 12.7s). Any 504 report windowed from a deploy will overstate the problem, and two external ones did exactly that.

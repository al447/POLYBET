...
> **Last updated:** 2026-08-24 (**fifth** external 504 report validated and rejected — its five fixes were already done, inapplicable, or would have broken the deploy; see "Report #5" in [improvement.md](improvement.md). Real cause found: `warm-cache.mjs` covered under 1% of the cache keyspace and **no RSC variants at all** — now fixed. Two new reading rules there: Worker telemetry cannot falsify a 504 here, and check the hourly split before believing a rate. Next lever is the `open-next.config.ts` queue, whose stated precondition — the `gamma.ts` payload projection — has now landed.) · **Phase:** Milestone 1 substantially complete (auth, geo-gate, fee engine, Deposit Wallet + deposit UI, pre-trade authorization, client-side market-order signing all built and verified on local workerd) · Weeks 2–4 (**Market Discovery, Trading Engine & WebSockets, Portfolio/Testing & Launch**) now being executed as **one combined build phase** — see the Milestones section below · **Repo:** on `feat/copy-trade`
>
> **Copy trading (FR-8) — development-complete 2026-08-19. One thing outstanding: a live mainnet fill.** The last open code gap closed in the same pass (`MIN_COPY_USD` $1 → $5 plus a click-time read of each market's own `minOrderSize` — see the copy-engine trap). Docs brought in line: [srs.md](srs.md) FR-8 now carries per-requirement status, **OI-5 is narrowed** to hands-off auto-signing only (not built, not needed), **OI-3 is moot** (no daemon exists), and [implementation.md §8.2](implementation.md) is a record rather than a proposal. ⚠️ **Built and unit-tested is not verified** — nothing here has been exercised against a real fill. The acceptance criterion is in [srs.md §10](srs.md); testing is pending a client deposit.
>
> **The "Build readiness" panel is gone from the home page (2026-08-20).** `components/status/readiness-panel.tsx` was deleted along with its now-empty directory — it was a build-phase instrument (its own docstring said "remove or gate this before public launch"), and it put our provisioning state and internal `P-*` ticket IDs on a public page above the markets. **Provisioning state is now read from `GET /api/health` only**, which returns the same `mode` + secret-presence data. Remember it still does not cover `POLYGON_RPC_URL` (P-9) — that gap predates this removal.
>
> **🟡 Verified builder tier — APPLIED 2026-08-23, awaiting reply.** Emailed `builder@polymarket.com` with the P-2 API key, the live URL `https://polybets.xyz`, a stated **$50k–250k/month** first-quarter projection, and the reason 10,000 relay tx/day is needed (Deposit Wallet deployments cap **onboarding**, not order throughput). Approval takes a few business days, is outside our control, and **gates public launch** — Step 4.7's go/no-go stands until it lands. ⚠️ **Our builder code has zero attributed volume** (measured 2026-08-23 across all four windows of `/v1/builders/volume`), and Polymarket's own upgrade steps put *"demonstrate consistent usage"* before the email. The mainnet smoke order ([builder-account-actions.md](builder-account-actions.md) item 4) is now the priority — it is the mitigation, and the only route to a programmatic confirmation later. Full record in [builder-account-actions.md](builder-account-actions.md) item 2.
>
> **✅ `polybets.xyz` IS LIVE and serving this Worker — corrected 2026-08-23.** The two paragraphs below said the custom domain was unwired and the app was on `*.workers.dev`. That was stale. Verified directly: DNS on Cloudflare (`jose.ns`/`nina.ns`), `https://polybets.xyz/` returns 200 with `x-opennext: 1` and `<title>Polybets — Prediction Markets</title>`, and `/api/health` reports `mode: "live"` with all five secrets present, `builder.configured: true`, `feeBps {taker:50, maker:0}`, `problems: []`. ⚠️ This confirms **the domain resolves and serves the app in live mode — nothing more.** [deployment.md §5](deployment.md#5-post-deploy-verification)'s nine post-deploy checks are still unrun, so this is not a verified production deployment.
>
...
>
> - **P-7 — resolved 2026-08-09.** Workers Paid active, first deploy succeeded, secrets pushed, build-time vars set. Remaining: run §5 verification (the custom domain is done — see P-8).
> - **P-8 (domain) — ✅ resolved 2026-08-23.** Production domain **`POLYBETS.XYZ`** is delegated to Cloudflare and serving this Worker — see the correction above for the evidence. It was used as the live-demo URL in the Verified tier application. *(This entry previously read "not yet independently verified"; the check has now been run.)*
...
src/
  ...
  lib/
    ...
    budget.ts                                   withBudget — hard ceiling on one await inside a Suspense boundary (built 2026-08-23); used by FeaturedHero, DiscoverySection, /market/[slug], /predict-ai. ⚠️ insurance, not a fix — see the setTimeout trap
...
scripts/
  ...
  warm-cache.mjs           post-deploy cache warmup (built 2026-08-23) — runs automatically at the end of `npm run deploy`; never fails the deploy. Warms HTML **and** RSC variants since 2026-08-24. ⚠️ a mitigation, not a fix: ~46 entries against a ~3,510-entry keyspace — see its docstring
...
improvement.md             504 investigation + fixes — read before diagnosing a timeout
```

**Commands:** `npm run dev` · `lint` · `typecheck` · `test` · `build` · `check:secrets` · `preview` · `deploy` · `warm` · `smoke:builder` · `builder:provision`
...
| [improvement.md](improvement.md) | 504/performance investigation and fixes — read its "How to read a 504 report" before acting on any timeout report |
...
**1 decision remains blocking** (full list in [srs.md §9](srs.md)):

| ID | Blocking question |
|---|---|
| **OI-1** | Is the target market inside the close-only geoblock list (US/UK/EU)? **The only genuinely blocking item left** — it changes the business case, not the code. |
...
| ~~OI-5~~ | ⚠️ **Narrowed 2026-08-19, not closed.** The shipped feature needs no legal call: it watches and sizes, then waits for a click, and every order is signed in-browser by the user's own key — no delegation, no server-held signing authority. What still needs a product + legal answer is **hands-off auto-signing**, which was deliberately not built. Do not treat "narrowed" as licence to remove the click. |
| ~~OI-3~~ | ✅ **Moot 2026-08-19.** There is no daemon. `useCopyEngine` polls in the user's own browser tab, so nothing needs a host, a budget line or a Durable Object. Cost: copies run only while the tab is open. |
...
**100/day cannot support a public launch.** ✅ **Verified application submitted 2026-08-23** (see the banner at the top and [builder-account-actions.md](builder-account-actions.md) item 2) — approval is externally controlled, takes a few business days, and gates launch.

---

## Traps

**🚩 504s: read [improvement.md](improvement.md) before diagnosing, and start with its "How to read a 504 report" section.** Resolved 2026-08-23 after a full day. Four things from it that will otherwise be re-derived:

- **Every deploy orphans the entire R2 cache.** OpenNext's `getR2Key` composes every key with `OPEN_NEXT_BUILD_ID`, so publishing invalidates everything at once and the first visitor to each route rebuilds inline. **A 504 burst after a deploy is expected and self-limiting.** Any 504 report windowed from a deploy will overstate the problem, and **three** external ones did exactly that — most recently report #5, which averaged a burst into a "7.0% error rate" when the steady state was ~0.06%.
  ⚠️ **`warm-cache.mjs` does NOT prevent this — corrected 2026-08-24.** Measured on the 17:00Z deploy: **134 of 135 504s** landed in the two hours after it, and **7 of the 8 worst-hit market slugs were already in the script's warm list**. Coverage is ~46 entries against a browse keyspace of ~3,510 (13 categories × 5 sorts × 27 range combos × 2 variants). The real lever is a `queue` in `open-next.config.ts` — read that file's docstring first, the override was tried and reverted once.
- **A `setTimeout` ceiling cannot be trusted to fire in a CPU-starved isolate.** `FeaturedHero`'s 8s `Promise.race` did not resolve within 25s — timers queue behind the work they are meant to bound. **A ceiling is insurance for work that is *waiting*, never a fix for work that is *burning CPU*.** This misread cost the most time of anything in the investigation; the giveaway was a stream capture whose `null` boundary was `GeoBanner` (first in document order), not the component being reasoned about.
- **`notFound()` cannot set a 404 from inside a `<Suspense>` boundary.** The response is `Transfer-Encoding: chunked`, so the status line flushes with the shell — a nonexistent market rendered the correct 404 UI under an HTTP **200**. Anything determining HTTP status must complete *before* the first byte. Relevant here because scanners actively probe this site.
- **A Server Component and its API route can silently diverge on query params.** `DiscoverySection` and `/api/markets` called the same `getCachedEvents` with different arguments: separate cache entries, a mismatched cursor, and a first paint of **21 of 24 closed events** — invisible because `MarketGrid` refetched with the right filters and painted over it.

- **`Failed to revalidate stale page <path>` is expected log noise, not a timeout.** With no `queue`, OpenNext defaults to `"dummy"`, whose `send()` throws immediately; `revalidateIfRequired` catches and logs it. No fetch, no hang, response unaffected. It fires on every stale serve of every route, correlates with slow requests without causing them, and a fourth external report built its entire (wrong) root cause on it. Trap 12 in [improvement.md](improvement.md).

**⚠️ `open-next.config.ts` is bare `r2IncrementalCache` on purpose.** `withRegionalCache` and `memoryQueue` were added and reverted the same day; the file's own docstring has the evidence. Do not re-add either from improvement.md's Deploy 2 section without reading it. Consequence to remember: with no `queue`, **background revalidation never happens at all** — an entry goes fresh → stale → expired and is only ever rebuilt by a visitor paying for it inline, once it is past `expire`. So `expire` is the real refresh interval, not a safety net, and a low-traffic route wants a *wider* one (`/predict-ai` sits at 3600 for this reason). *(This previously read "revalidation runs inside the visitor's request", which understated it — nothing revalidates in the background, so the browse caches' `revalidate: 300` is not what bounds staleness.)*
...
**🚩 Our OWN builder attribution is readable only through the public volume board, and only once we have volume.** A different endpoint family from the trader leaderboard above — verified 2026-08-23 against `docs.polymarket.com/api-reference/builders/*`:

```
GET data-api.polymarket.com/v1/builders/volume?timePeriod=DAY|WEEK|MONTH|ALL
GET data-api.polymarket.com/v1/builders/leaderboard?timePeriod=ALL&limit=3
```

Both return a bare array of `{rank, builder, builderCode, builderLogo, verified, volume, activeUsers}` — `rank` is a **string** here too. `timePeriod` is again the parameter, and there is no per-builder lookup: **`/v1/builders/<code>`, `/v1/builders/profile?builderCode=…` and `clob.polymarket.com/builder/<code>` all 404.**

The consequence is the important part: **`verified` is the only machine-readable proof of our tier, and it sits on a row that does not exist until our code has traded.** Measured 2026-08-23, our code is absent from all four windows — including `ALL`'s 611 rows — so until the first mainnet fill lands there is no way to confirm a granted tier upgrade except Polymarket's reply email. That is the argument for running the smoke order now rather than deferring it. Scale, for calibration: rank 1 `betmoar` shows $1.87B all-time across 3,185 active users; rank 3 does ~$58M/month.
...
- **✅ The order minimum is in SHARES, not dollars — fixed 2026-08-19.** The CLOB's `min_order_size` is **5 shares**, which is $2.50 at a price of 0.50 but $4.50 at 0.90, so no dollar figure clears it at every price except by construction. `MIN_COPY_USD` was `1`, and a $1–2 copy was rejected upstream and landed as a red `failed` row carrying a raw SDK message — indistinguishable from a broken feature. Two layers now:
  - **`MIN_COPY_USD = 5`.** Shares are `usd / price` and a price never exceeds `1.00`, so **$5 buys ≥ 5 shares at any price**. Arithmetic, not a chosen round number — don't "tidy" it back down. `FollowDialog` enforces it on both the fixed size and the per-trade cap.
  - **The market's own floor, read at click time.** `readMinOrderShares` (`execute.ts`) pulls `minOrderSize` off `fetchOrderBook` — it is per-market and can exceed 5 — and `checkCopyPreconditions` skips as `below_minimum`. Same three-state convention as `marketOpen`: `undefined` never looked up, `null` unreadable, and **only a real number blocks**. The buy side estimates shares at `expectedPrice` (the price the queued row already shows), reading a missing anchor as `1.00` so an unreadable row errs toward being allowed through rather than refused on a guess.

  `decideCopy` is deliberately *not* where this lives — same reasoning as `marketClosed`: the poll loop has no fresh answer, and fetching a book per candidate intent per tick would spend requests to be wrong more slowly. The click is where a fresh answer exists.
...
| Copy trade feature | **Development-complete as of 2026-08-19 — no open code items.** The engine watches, sizes and queues; the user places each copy with one click. The last gap (order minimum in shares, not dollars) closed the same day. `/leaderboard` is a complete feature. Docs now agree: [srs.md](srs.md) FR-8 carries per-requirement status, [implementation.md §8.2](implementation.md) is a record not a proposal. OI-5 **narrowed** (no delegation, no server-held key, every order behind a human click — hands-off auto-signing deliberately not built), OI-3 **moot** (no daemon). ⚠️ **Built and unit-tested is not verified** — **not yet confirmed against a real mainnet fill**, pending a client deposit. See the copy-trading trap below and the FR-8 acceptance criterion in [srs.md §10](srs.md). |
...
| Domain `POLYBETS.XYZ` → Cloudflare | ✅ **Done and verified 2026-08-23.** Delegated to Cloudflare and serving this Worker in live mode (P-8). *(Previously recorded here as "supplied, unverified" — the check has now been run.)* |
| Up to 4 revisions | **Commercial term, recorded** in srs.md §8 — no code impact. |

**Corrected 2026-08-19 — one real gap remains, not two.** This line previously named copy trading and sports AI as the two gaps. **Copy trading was subsequently built** (a narrower product than the SRS specified: click-gated, in-browser, no delegation — which is exactly why it turned out to be affordable inside the original engagement). **Sports AI (FR-7) is the remaining gap**, still excluded for the same reason as before: no model, provider, data source or budget is defined, so it is not estimable as written. Revisit as a separately scoped, separately priced Phase 2 once a provider and budget are chosen (see [implementation.md §8](implementation.md)).
